...
const Login = (props) => {
    ...
    const onSubmit = async (data) => {
        setLoading(true);
        let res = await fetch(server_uri + "api/v1/user/login", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(data) });
        setLoading(false);
    ...
    return (
        <>
            ...
            <div className="mx-auto max-w-2xl m-10 mb-10 bg-white">
                <div className='flex flex-col border border-black rounded-3xl justify-center p-5 gap-5 shadow-2xl'>
                    ...
                    <div className="inline-flex items-center justify-center w-full">
                        <hr className="w-64 h-px my-3 bg-blue-900 border-0" />
                        <span className="absolute px-3 font-medium text-gray-900 -translate-x-1/2 bg-white left-1/2">LOGIN</span>
                    </div>
                    <form onSubmit={handleSubmit(onSubmit)} className='flex flex-col justify-center items-center'>
                        ...
                        <input disabled = {loading} type="submit" value="Login" className='bg-blue-700 px-3 py-1 text-center text-white hover:bg-blue-400 border border-blue-900 rounded-md cursor-pointer m-5' />
                    </form>
